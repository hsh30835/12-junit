...
public class EnvAnnotationTests {
    ...
    * 이 떄, 특정 OS에서 무시되는 사유를 disabledReason에 기술하여
    ...
    * */
    ...
    @Test
    @EnabledOnOs(value = {OS.WINDOWS, OS.LINUX}, disabledReason = "윈도우와 리눅에서만 테스트를 합니다.")
    ...
    * @DisableOnJre를 이용하여 특정 JRE 버전에서만 테스트를 Disable 하는 것도 가능하다.
    ...
    * */
    ...
    /*
    * 운영체재의 시스템 환경변수에 따라 테스트를 활성화 또는 비활성화 할 수 있다.
    * cmd에서 set명령으로 시스템 환경 변수값을 확인해 볼 수 있다.
    * 시스템 변수의 이름은 대소문자를 구분하지 않지만 값은 대소문자룰 구분한다.
    * */
    @Test
    @EnabledIfEnvironmentVariable(named = "username", matches = "jinno")
